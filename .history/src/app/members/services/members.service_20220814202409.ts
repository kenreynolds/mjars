...
export class MembersService {
  ...
  // NEW: Node.js API
  createMember(postData: any): Observable<any> {
    // TODO: Refactor so that subscription happens here.
    return this.http
  ...
  deleteMember(memberId: string): Observable<any> {
    return this.http
      .delete(`http://localhost:3000/api/members/${memberId}`)
  }

  getMembers(): Observable<any> {
    return this.http
      .get<{ message: string, members: any }>('http://localhost:3000/api/members');
  }
